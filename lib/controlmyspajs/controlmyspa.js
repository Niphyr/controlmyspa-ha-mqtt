...
class ControlMySpa {
  ...
  async getDefaultSpa () {
    ...
    if (req.status === 200) {
      ...
      // Store all spa IDs and default to first
      this.allSpaIds = body.spas.map(s => s._id);
      this.currentSpaId = body.spas[0]._id;

      logDebug('Default spa id: ' + this.currentSpaId + ', all spa ids: ' + JSON.stringify(this.allSpaIds));
  ...
  getComponentByPort(componentType, port, spaData = this.currentSpa) {
    return spaData.components.find(component => {
      return component.componentType === componentType && component.port === `${port}`;
    });
  }

  async waitForComponentState(componentType, port, desiredState, maxAttempts = 8, delayMs = 1000) {
    let latestSpaData = this.currentSpa;

    for (let attempt = 1; attempt <= maxAttempts; attempt++) {
      await this.sleep(delayMs);
      latestSpaData = await this.getSpa();

      const currentComponent = this.getComponentByPort(componentType, port, latestSpaData);
      if (currentComponent?.value === desiredState) {
        logInfo(`${componentType} ${port} reached ${desiredState} after ${attempt} poll${attempt === 1 ? '' : 's'}`);
        return latestSpaData;
      }
    }

    const currentComponent = this.getComponentByPort(componentType, port, latestSpaData);
    logInfo(`${componentType} ${port} did not reach ${desiredState} after ${maxAttempts} polls, current state ${currentComponent?.value}`);
    return latestSpaData;
  }
    ...
    async setComponentState(logicalType, deviceNumber, desiredState) {
      ...
      const typeMap = {
        ...
        try {
          ...
          const responseBody = err.response?.data ? JSON.stringify(err.response.data) : 'no response body';
          logError(`Failed to set ${logicalType} state with payload ${JSON.stringify(commandPayload)}: ${responseBody}`);
          throw err;
        }

        if (req.status === 200) {
          const oldState = this.getComponentByPort(hardwareType, deviceNumber);

            if (this.waitForResult) {
            const newSpaData = await this.waitForComponentState(hardwareType, deviceNumber, desiredState);
            const newState = this.getComponentByPort(hardwareType, deviceNumber, newSpaData);
  ...
  /**
   * Create a new ControlMySpa instance for a different spa on the same account.
   * Copies all authentication state so a second login is not required.
   */
  createForSpa(spaId) {
    const client = new ControlMySpa(this.email, this.password, this.celsius);
    client.tokenData = { ...this.tokenData };
    client.userInfo = this.userInfo;
    client.mobileClientId = this.mobileClientId;
    client.mobileClientSecret = this.mobileClientSecret;
    client.tokenEndpoint = this.tokenEndpoint;
    client.refreshEndpoint = this.refreshEndpoint;
    client.whoami = this.whoami;
    client.currentSpaId = spaId;
    client.allSpaIds = [...this.allSpaIds];
    client.waitForResult = this.waitForResult;
    client.scheduleFilterIntervalEnum = this.scheduleFilterIntervalEnum;
    return client;
  }
