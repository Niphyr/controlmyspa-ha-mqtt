...
class Spa extends EventEmitter {
  ...
  initPreAuthenticated() {
    this.client.getSpa()
    .then(s => {
      this.initialized = true;
      this.emit('initialized');
      this.emit('status_updated');
    }).catch(err => logError(`Failed to initialize pre-authenticated spa client: ${err}`));
  }
  ...
  getComponentByPort(type, port) {
    return this.getComponents(type).find(component => `${component.port}` === `${port}`);
  }

  setLocalComponentState(type, port, state) {
    const component = this.getComponentByPort(type, port);
    if (component) {
      component.value = state;
    }
  }

  confirmComponentState(type, port, desiredState, attemptsRemaining = 6) {
    let self = this;
    ...
    setTimeout(() => {
      self.client.getSpa().then(() => {
        const component = self.getComponentByPort(type, port);
        if (component && component.value === desiredState) {
          self.emit('status_updated');
          return;
        }

        self.confirmComponentState(type, port, desiredState, attemptsRemaining - 1);
      }).catch(err => {
        logError(`Failed to confirm ${type} ${port} state ${desiredState}: ${err}`);
        self.confirmComponentState(type, port, desiredState, attemptsRemaining - 1);
      });
    }, self.refreshAfterUpdate);
  }
  ...
  setLightState(light, state) {
    ...
    self.client.setComponentState("light", light, state).then(res => {
      ...
      if (res) {
        self.setLocalComponentState("LIGHT", light, state);
        this.emit('status_updated')
      }
      self.confirmComponentState("LIGHT", light, state);
  ...
  setBlowerState(blower, state) {
    ...
    self.client.setComponentState("blower", blower, state).then(res => {
      ...
      if (res) {
        self.setLocalComponentState("BLOWER", blower, state);
        this.emit('status_updated')
      }
      self.confirmComponentState("BLOWER", blower, state);
    }).catch(err => logError(`Failed to set blower ${blower} state ${state}: ${err}`))
  }

  setJetState(jet, state) {
    let self = this;
    self.client.setComponentState("pump", jet, state).then(res => {
      logDebug(`Set jet ${jet} state ${state} complete: ${JSON.stringify(res)}`)
      if (res) {
        self.setLocalComponentState("PUMP", jet, state);
        this.emit('status_updated')
      }
      self.confirmComponentState("PUMP", jet, state);
